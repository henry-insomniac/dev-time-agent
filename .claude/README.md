...
`.claude` 目录保存 `dev-time-agent` 的长期上下文，供人类维护者和 Agent 在开发、排障、评审时快速理解项目约定。
...
- `coding-standards.md`：Agent Runtime 编码规范、workflow 约束、行数约束和评审检查项。
...
- 调整代码组织、文件职责、函数职责、workflow 边界或行数上限时，同步更新 `coding-standards.md`。

## 当前状态

本目录由脚手架在 2026-06-11 初始化，并同步 Dev Time 产品 PRD、技术架构和 `dev-time-agent` Agent Runtime 架构草案。请根据真实实现继续补充技术栈和验证命令。
